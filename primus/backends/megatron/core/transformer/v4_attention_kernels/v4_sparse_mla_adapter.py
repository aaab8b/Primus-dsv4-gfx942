...
class _V4SparseMLACSAFn(torch.autograd.Function):
    ...
    def forward(  # type: ignore[override]
        ...
        topk_g = _pad_topk_64(
            _build_csa_topk(topk_idxs, S, Skv, P, W, int(cp_dwindow), int(cp_global_start))
        )
    ...
    def backward(ctx, grad_o_bh: torch.Tensor):  # type: ignore[override]
        ...
        return (
            dq_bh, dk_local, dv_local, dpool.to(dq_bh.dtype), None, dsink_out,
...
class _V4SparseMLAAttnFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_o_bh: torch.Tensor):  # type: ignore[override]
        ...
        #                cp_dwindow, cp_global_start, k_is_latent, fwd_fn, bwd_fn)
        return (
            dq_bh, dk_bh, dv_bh, dsink_out, None, None, None, None,
    ...
    def _csa_from_pool(
        ...
        *,
        ...
        return _V4SparseMLACSAFn.apply(
            ...
            int(cp_dwindow), int(cp_global_start), bool(k_is_latent), fwd_fn, bwd_fn,
        )
