    ...
    for target in args.lengths:
        ...
        with open(path, "w", encoding="utf-8") as f:
            for _ in range(args.rows):
                ...
                f.write(json.dumps({"instruction": "Continue the following document.",
                                    "input": "", "output": text}, ensure_ascii=False) + "\n")
        ...
        print(f"[data] [{ok}] {name}: {args.rows} rows, "
              f"{os.path.getsize(path)/2**20:.1f} MB, row0={ntok} tokens (target {target})", flush=True)
